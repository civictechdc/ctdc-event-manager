...
`opencode.jsonc` intentionally uses OpenCode built-in LSP support for TypeScript, YAML, and Bash. Do not add custom language-server commands or enable Deno unless the project gains matching source/configuration and the dependency ownership is documented.
